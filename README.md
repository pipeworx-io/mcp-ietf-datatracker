...
`tools/list` at `https://gateway.pipeworx.io/ietf-datatracker/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/rfc \
  -H 'Content-Type: application/json' \
  -d '{"number":9000}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/rfc`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "ietf-datatracker": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-ietf-datatracker"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-ietf-datatracker
```
